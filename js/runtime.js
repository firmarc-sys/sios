(() => {
  ...
  const CONFIG = Object.freeze({
    version:'3.1.1', system:'jahorin-mercury', ownerGid:'399152573423', ownerMode:'Prime Orchestrator',
    ari:'https://ari-689058655022.us-west1.run.app', runtime:'https://agentic-mercury-runtime-689058655022.us-west1.run.app',
    routes:{health:'/health',ready:'/ready',identity:'/api/identity',session:'/api/identity/session',render:'/api/render-state',iot:'/api/iot',syncori:'/api/syncori',tae:'/api/tae',runtime:'/api/runtime'},
  ...
  async function request(path, options={}){
    ...
    try{
      ...
      if(!res.ok) throw new Error(data?.error||data?.detail||data?.message||`ARI HTTP ${res.status}`);
      state.online=true; return data;
    } finally {clearTimeout(timer)}
  }
  async function identity(){
    try{
      const raw=await request(CONFIG.routes.identity,{timeout:7000});
      state.identity={...raw,gid:raw?.gid||CONFIG.ownerGid,mode:raw?.mode||raw?.role||CONFIG.ownerMode,authenticated:Boolean(raw?.authenticated??raw?.identityAccepted)};
      return state.identity;
    }catch{state.identity={gid:CONFIG.ownerGid,mode:CONFIG.ownerMode,authenticated:false};return state.identity}
  }
  ...
  async function tae(prompt='TAE, enter Demo Mode'){const raw=await request(CONFIG.routes.tae,{method:'POST',body:{prompt,request_id:uuid()}});return {...raw,message:raw?.message||raw?.response||raw?.reply?.text||'This is not an app. This is me.'}}
  ...
  async function status(){const el=document.querySelector('.status');if(!el)return;try{const id=await identity();el.dataset.online='true';el.textContent=`ARI · online · GID ${id.gid||CONFIG.ownerGid}`;return id}catch(e){el.dataset.online='false';el.textContent='ARI · offline';return null}}
